<?php
// AJAX endpoint: delete contingent
...
set_exception_handler(function($e){
    ...
    error_log('[ajax/contingent_delete][exception] ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
...
register_shutdown_function(function(){
    ...
    if ($err) {
        http_response_code(500);
        error_log('[ajax/contingent_delete][shutdown] ' . $err['message'] . ' in ' . $err['file'] . ':' . $err['line']);
...
if ($id <= 0) {
    ...
    echo json_encode(['success' => false, 'message' => 'ID kontinjen tidak sah.']);
    exit;
}
...
try {
    ...
    if ($result['success']) {
        echo json_encode(['success' => true, 'message' => 'Kontinjen berjaya dipadam.']);
    } else {
        http_response_code(400);
        echo json_encode($result);
    }
} catch (Exception $e) {
    error_log('[ajax/contingent_delete] ' . $e->getMessage() . " in " . $e->getFile() . ':' . $e->getLine());
    error_log($e->getTraceAsString());
    http_response_code(500);
    $msg = (defined('DEBUG_MODE') && DEBUG_MODE) ? $e->getMessage() : 'Ralat memadam rekod kontinjen.';
    echo json_encode(['success' => false, 'message' => $msg]);
    exit;
}
